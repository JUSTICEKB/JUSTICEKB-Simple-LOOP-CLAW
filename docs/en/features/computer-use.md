...
```bash
loop-claw --no-computer-use
CLAUDE_COMPUTER_USE_ENABLED=0 loop-claw
```

You can also write the global config file at `~/.claude/simple-loop-claw/computer-use-config.json`:
