...
```bash
./bin/loop-claw                          # Interactive TUI mode
./bin/loop-claw -p "your prompt here"    # Headless mode
./bin/loop-claw --help                   # Show all options
```
...
```bash
CLAUDE_CODE_FORCE_RECOVERY_CLI=1 ./bin/loop-claw
```
