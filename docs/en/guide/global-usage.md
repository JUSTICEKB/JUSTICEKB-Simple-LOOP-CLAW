...
If you want to run `loop-claw` directly from any project directory, set up one of the following. Once configured, `loop-claw` will automatically recognize your current working directory.
...
If `loop-claw` runs on Windows / Git Bash but tools such as Node, Python, uv, or bun are installed inside WSL, call them through WSL explicitly:

```bash
wsl -e bash -lc 'node --version && python3 --version'
```

When simple-loop-claw detects `wsl` / `wsl.exe`, it automatically sets `MSYS2_ARG_CONV_EXCL=*` so Git Bash does not rewrite WSL paths such as `/home/...` into `C:/Program Files/Git/home/...`.
