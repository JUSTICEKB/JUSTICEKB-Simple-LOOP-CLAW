...
如果你希望在任意项目目录直接运行 `loop-claw`，可以通过以下方式配置。配置完成后，`loop-claw` 会自动识别你当前所在的工作目录。
...
如果 `loop-claw` 运行在 Windows / Git Bash，但 Node、Python、uv、bun 等工具主要安装在 WSL 里，可以显式通过 WSL 调用：

```bash
wsl -e bash -lc 'node --version && python3 --version'
```

simple-loop-claw 会在检测到 `wsl` / `wsl.exe` 调用时自动设置 `MSYS2_ARG_CONV_EXCL=*`，避免 Git Bash 把 `/home/...` 这类 WSL 路径错误转换成 `C:/Program Files/Git/home/...`。
