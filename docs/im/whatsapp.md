...
WhatsApp 方案适合用自己的 WhatsApp 个人号在私聊里远程驱动 Simple LoopClaw。当前实现只处理个人私聊，不处理群聊、频道、状态广播。
...
它的工作方式更接近“把 Simple LoopClaw 作为一台已登录的 WhatsApp Web 设备挂到你的个人 WhatsApp 账号上”：
...
5. 已授权用户发来的消息会被转成 Simple LoopClaw session 输入
