...
describe('Electron sidecar manager', () => {
  ...
  it('passes portable config and adapter server URL through the sidecar env', () => {
    const configDir = mkdtempSync(path.join(tmpdir(), 'simple-loop-claw-config-'))
    try {
  ...
  it('persists and prioritizes preferred server ports from the config dir', () => {
    ...
    try {
      ...
      mkdirSync(path.join(configDir, 'simple-loop-claw'), { recursive: true })
      writeFileSync(
        path.join(configDir, 'simple-loop-claw', 'settings.json'),
