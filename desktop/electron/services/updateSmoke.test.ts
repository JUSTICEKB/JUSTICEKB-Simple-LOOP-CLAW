...
describe('Electron update smoke updater', () => {
  it('stays disabled unless a smoke version is configured', () => {
    ...
    expect(parseUpdateSmokeEnv({ LOOP_CLAW_ELECTRON_UPDATE_SMOKE_VERSION: '  ' })).toBeNull()
  })

  it('drives the real Electron updater service contract and logs the install signal', async () => {
    const tempDir = mkdtempSync(join(tmpdir(), 'simple-loop-claw-update-smoke-'))
    const logPath = join(tempDir, 'update-smoke.jsonl')
    try {
      const updater = createUpdateSmokeUpdaterFromEnv({
        LOOP_CLAW_ELECTRON_UPDATE_SMOKE_VERSION: '9.9.9-smoke',
        LOOP_CLAW_ELECTRON_UPDATE_SMOKE_BODY: 'Smoke notes',
        LOOP_CLAW_ELECTRON_UPDATE_SMOKE_TOTAL_BYTES: '200',
        LOOP_CLAW_ELECTRON_UPDATE_SMOKE_LOG: logPath,
      })
