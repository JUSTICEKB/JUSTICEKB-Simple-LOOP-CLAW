...
export type UpdateSmokeEnv = {
  LOOP_CLAW_ELECTRON_UPDATE_SMOKE_VERSION?: string
  LOOP_CLAW_ELECTRON_UPDATE_SMOKE_BODY?: string
  LOOP_CLAW_ELECTRON_UPDATE_SMOKE_TOTAL_BYTES?: string
  LOOP_CLAW_ELECTRON_UPDATE_SMOKE_LOG?: string
}
...
export function parseUpdateSmokeEnv(env: UpdateSmokeEnv): UpdateSmokeConfig | null {
  const version = env.LOOP_CLAW_ELECTRON_UPDATE_SMOKE_VERSION?.trim()
  if (!version) return null
  return {
    version,
    body: env.LOOP_CLAW_ELECTRON_UPDATE_SMOKE_BODY ?? 'Electron update smoke release',
    totalBytes: parsePositiveInteger(env.LOOP_CLAW_ELECTRON_UPDATE_SMOKE_TOTAL_BYTES, 100),
    logPath: env.LOOP_CLAW_ELECTRON_UPDATE_SMOKE_LOG,
  }
}
