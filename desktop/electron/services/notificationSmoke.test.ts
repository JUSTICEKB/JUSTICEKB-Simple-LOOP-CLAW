...
describe('Electron notification smoke hook', () => {
  ...
  it('schedules a targeted notification for Computer Use smoke validation', () => {
    ...
    expect(scheduleNotificationSmoke({
      env: {
        LOOP_CLAW_ELECTRON_NOTIFICATION_SMOKE_SESSION_ID: 'session-smoke',
        LOOP_CLAW_ELECTRON_NOTIFICATION_SMOKE_TITLE: 'Target smoke',
        LOOP_CLAW_ELECTRON_NOTIFICATION_SMOKE_BODY: 'Click target smoke',
        LOOP_CLAW_ELECTRON_NOTIFICATION_SMOKE_DELAY_MS: '10',
      },
      NotificationClass,
      onAction,
      setTimer: timer,
  ...
  it('can trigger an explicit synthetic action for packaged renderer ack smoke', () => {
    ...
    expect(scheduleNotificationSmoke({
      env: {
        LOOP_CLAW_ELECTRON_NOTIFICATION_SMOKE_SESSION_ID: 'session-smoke',
        LOOP_CLAW_ELECTRON_NOTIFICATION_SMOKE_TITLE: 'Target smoke',
        LOOP_CLAW_ELECTRON_NOTIFICATION_SMOKE_TRIGGER_ACTION: '1',
      },
      NotificationClass,
      onAction,
      setTimer: timer,
  ...
  it('writes JSONL smoke events when a log path is configured', () => {
    const tmp = mkdtempSync(path.join(tmpdir(), 'simple-loop-claw-notification-smoke-'))
  ...
  it('writes renderer acknowledgements to the notification smoke log only when configured', () => {
    ...
    expect(logNotificationSmokeRendererAck({
      LOOP_CLAW_ELECTRON_NOTIFICATION_SMOKE_LOG: logPath,
