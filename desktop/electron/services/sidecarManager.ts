...
/** Parse h5Access.fixedPort out of simple-loop-claw/settings.json contents. */
...
export function readH5FixedPort(env: NodeJS.ProcessEnv = process.env): number | null {
  try {
    const settingsPath = path.join(claudeConfigDir(env), 'simple-loop-claw', 'settings.json')
