...
function tempDir() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'simple-loop-claw-app-mode-'))
  tempDirs.push(dir)
  return dir
}

function app(root = tempDir()): AppModeAppLike & { root: string } {
  const exe = path.join(root, 'Simple LoopClaw.app', 'Contents', 'MacOS', 'Simple LoopClaw')
...
describe('Electron app mode service', () => {
  ...
  it('sets portable environment variables before sidecars start', () => {
    ...
    expect(env.LOOP_CLAW_APP_PORTABLE_DIR).toBe('1')
  ...
  it('returns the active app mode shape expected by settingsStore', () => {
    ...
    expect(getAppMode(fakeApp, { CLAUDE_CONFIG_DIR: '/portable', LOOP_CLAW_APP_PORTABLE_DIR: '1' })).toMatchObject({
      mode: 'portable',
