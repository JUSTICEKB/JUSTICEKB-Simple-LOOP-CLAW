...
function tempDir() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'simple-loop-claw-terminal-'))
  tempDirs.push(dir)
  return dir
}
...
describe('Electron terminal service', () => {
  ...
  it('copies packaged node-pty to a writable runtime cache and restores helper executable bits', () => {
    ...
    expect(fs.existsSync(path.join(cache, '.simple-loop-claw-node-pty-manifest.json'))).toBe(true)
  })
  ...
  itOnDarwin('removes stale macOS quarantine attributes from the node-pty runtime cache', () => {
    ...
    execFileSync('/usr/bin/xattr', ['-w', 'com.apple.quarantine', '0381;00000000;Chrome;SIMPLE-LOOP-CLAW-TEST', cachedEntry])
