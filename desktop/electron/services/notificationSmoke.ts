...
export function logNotificationSmokeRendererAck(env: NodeJS.ProcessEnv, payload: unknown): boolean {
  const logPath = env.LOOP_CLAW_ELECTRON_NOTIFICATION_SMOKE_LOG?.trim()
...
export function scheduleNotificationSmoke({
  ...
  const sessionId = env.LOOP_CLAW_ELECTRON_NOTIFICATION_SMOKE_SESSION_ID?.trim()
  if (!sessionId) return false

  const title = env.LOOP_CLAW_ELECTRON_NOTIFICATION_SMOKE_TITLE?.trim() || 'Simple LoopClaw notification smoke'
  const body = env.LOOP_CLAW_ELECTRON_NOTIFICATION_SMOKE_BODY?.trim() || 'Click to return to the target session.'
  const delayMs = parseNotificationSmokeDelay(env.LOOP_CLAW_ELECTRON_NOTIFICATION_SMOKE_DELAY_MS)
  const triggerSyntheticAction = shouldTriggerSyntheticAction(env.LOOP_CLAW_ELECTRON_NOTIFICATION_SMOKE_TRIGGER_ACTION)
  const logPath = env.LOOP_CLAW_ELECTRON_NOTIFICATION_SMOKE_LOG?.trim()
