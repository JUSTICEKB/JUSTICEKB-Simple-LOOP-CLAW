...
fn dir_has_portable_data(dir: &Path) -> bool {
    ...
    [
        ...
        || dir.join("simple-loop-claw").is_dir()
}
...
fn get_app_mode(app: AppHandle) -> serde_json::Value {
    ...
    let config_dir_source = if env_config_dir.is_some() {
        if std::env::var_os("LOOP_CLAW_APP_PORTABLE_DIR").is_some() {
            "portable"
        } else {
            "environment"
        }
...
fn window_state_path(app: &AppHandle) -> Option<PathBuf> {
    ...
    // %APPDATA%\com.simple-loop-claw.desktop\.
...
fn server_state_path() -> Option<PathBuf> {
    // Lives next to simple-loop-claw/settings.json (CLAUDE_CONFIG_DIR or ~/.claude) so
...
fn setup_system_tray(app: &mut tauri::App) -> tauri::Result<()> {
    let menu = MenuBuilder::new(app)
        .text(TRAY_SHOW_ID, "Show Simple LoopClaw")
        .separator()
        .text(TRAY_QUIT_ID, "Quit Simple LoopClaw")
        .build()?;

    let mut tray = TrayIconBuilder::with_id("main-tray")
        .tooltip("Simple LoopClaw")
        .menu(&menu)
...
/// 解析 simple-loop-claw/settings.json 里的 h5Access.fixedPort。范围必须与
...
fn read_h5_fixed_port() -> Option<u16> {
    let path = claude_config_dir()?.join("simple-loop-claw").join("settings.json");
...
pub fn run() {
    ...
    let builder = builder
        .menu(|app| {
            let about_item =
                MenuItemBuilder::with_id("nav_about", "关于 Simple LoopClaw").build(app)?;
            let settings_item = MenuItemBuilder::with_id("nav_settings", "设置...")
                .accelerator("CmdOrCtrl+,")
                .build(app)?;

            let app_submenu = SubmenuBuilder::new(app, "Simple LoopClaw")
                .item(&about_item)
                .separator()
                .item(&settings_item)
                .separator()
