...
describe('doctorRepair', () => {
  ...
  it('clears only the safe desktop UI storage keys', () => {
    ...
    window.localStorage.setItem('simple-loop-claw-chat-history', 'preserve')
    window.localStorage.setItem('simple-loop-claw-provider-config', 'preserve')
    ...
    expect(window.localStorage.getItem('simple-loop-claw-chat-history')).toBe('preserve')
    expect(window.localStorage.getItem('simple-loop-claw-provider-config')).toBe('preserve')
  })
  ...
  it('keeps local repair successful when the server doctor endpoint is unavailable', async () => {
    ...
    expect(result.local.removedKeys).toContain('simple-loop-claw-theme')
