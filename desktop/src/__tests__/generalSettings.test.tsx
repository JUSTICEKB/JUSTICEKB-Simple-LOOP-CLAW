...
describe('Settings > General tab', () => {
  beforeEach(() => {
    ...
    tauriDialogMock.open.mockResolvedValue('/Users/test/simple-loop-claw-data')
    ...
    useSettingsStore.setState({
      ...
      traceCapture: { enabled: true, storageDir: '/Users/test/.claude/simple-loop-claw/traces' },
      ...
      appMode: {
        mode: 'default',
        portableDir: null,
        defaultPortableDir: '/Applications/Simple LoopClaw/CLAUDE_CONFIG_DIR',
      ...
      setAppMode: vi.fn().mockImplementation(async (mode: AppMode, portableDir?: string | null) => {
        useSettingsStore.setState({
          appMode: {
            mode,
            portableDir: mode === 'portable' ? portableDir ?? '/Applications/Simple LoopClaw/CLAUDE_CONFIG_DIR' : null,
            defaultPortableDir: '/Applications/Simple LoopClaw/CLAUDE_CONFIG_DIR',
            activeConfigDir: mode === 'portable' ? portableDir ?? '/Applications/Simple LoopClaw/CLAUDE_CONFIG_DIR' : null,
  ...
  it('lets desktop users choose a portable data directory and relaunch immediately', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByLabelText('Portable data directory')).toHaveValue('/Users/test/simple-loop-claw-data')
    })
    ...
    await waitFor(() => {
      expect(useSettingsStore.getState().setAppMode).toHaveBeenCalledWith('portable', '/Users/test/simple-loop-claw-data')
  ...
  it('switches back to the system directory without deleting portable data', async () => {
    useSettingsStore.setState({
      appMode: {
        mode: 'portable',
        portableDir: '/Users/test/simple-loop-claw-data',
        defaultPortableDir: '/Applications/Simple LoopClaw/CLAUDE_CONFIG_DIR',
        activeConfigDir: '/Users/test/simple-loop-claw-data',
  ...
  it('validates portable directory input and lets users reset to the app-side folder', async () => {
    ...
    expect(input).toHaveValue('/Applications/Simple LoopClaw/CLAUDE_CONFIG_DIR')
  ...
  it('requests native notification permission when desktop notifications are enabled', async () => {
    ...
    expect(desktopNotificationsMock.notifyDesktop).toHaveBeenCalledWith({
      title: 'Simple LoopClaw notifications are enabled',
...
describe('Settings > About tab', () => {
  beforeEach(() => {
    ...
    useUpdateStore.setState({
      ...
      releaseNotes: '# Simple LoopClaw v0.1.5\n\n- Fixed updater rendering\n- Added markdown support',
      progressPercent: 0,
      downloadedBytes: 0,
      totalBytes: null,
      error: null,
  ...
  it('renders release notes with markdown formatting', async () => {
    ...
    expect(await screen.findByRole('heading', { name: 'Simple LoopClaw v0.1.5' })).toBeInTheDocument()
