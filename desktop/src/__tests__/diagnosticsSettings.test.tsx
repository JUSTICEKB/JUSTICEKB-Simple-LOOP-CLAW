...
describe('Settings > Diagnostics tab', () => {
  beforeEach(() => {
    ...
    diagnosticsApiMock.getStatus.mockResolvedValue({
      logDir: '/tmp/claude/simple-loop-claw/diagnostics',
      diagnosticsPath: '/tmp/claude/simple-loop-claw/diagnostics/diagnostics.jsonl',
      cliDiagnosticsPath: '/tmp/claude/simple-loop-claw/diagnostics/cli-diagnostics.jsonl',
      runtimeErrorsPath: '/tmp/claude/simple-loop-claw/diagnostics/runtime-errors.log',
      exportDir: '/tmp/claude/simple-loop-claw/diagnostics/exports',
      retentionDays: 7,
    ...
    diagnosticsApiMock.exportBundle.mockResolvedValue({
      bundle: {
        path: '/tmp/claude/simple-loop-claw/diagnostics/exports/simple-loop-claw-diagnostics.tar.gz',
        fileName: 'simple-loop-claw-diagnostics.tar.gz',
        bytes: 1024,
      },
    })
    ...
    doctorRepairMock.runDoctorRepair.mockResolvedValue({
      local: {
        removedKeys: ['simple-loop-claw-open-tabs', 'simple-loop-claw-session-runtime'],
        missingKeys: ['simple-loop-claw-theme', 'simple-loop-claw-locale', 'simple-loop-claw.persistence.schemaVersion'],
        failedKeys: [],
      },
      server: {
        ok: true,
  ...
  it('shows diagnostics status, actions, and recent events', async () => {
    ...
    expect(screen.getByText('/tmp/claude/simple-loop-claw/diagnostics')).toBeInTheDocument()
  ...
  it('exports a diagnostics bundle from the settings page', async () => {
    ...
    expect(await screen.findByText('/tmp/claude/simple-loop-claw/diagnostics/exports/simple-loop-claw-diagnostics.tar.gz')).toBeInTheDocument()
  })
  ...
  it('runs Doctor from Diagnostics without clearing unrelated desktop state', async () => {
    window.localStorage.setItem('simple-loop-claw-open-tabs', '{"activeTabId":"__settings__"}')
    window.localStorage.setItem('simple-loop-claw-theme', 'dark')
    window.localStorage.setItem('simple-loop-claw-chat-history', 'keep')
    ...
    expect(window.localStorage.getItem('simple-loop-claw-chat-history')).toBe('keep')
  })
})
