...
const PROJECT_ORDER_STORAGE_KEY = 'simple-loop-claw-sidebar-project-order'
const PROJECT_PINNED_STORAGE_KEY = 'simple-loop-claw-sidebar-pinned-projects'
const PROJECT_HIDDEN_STORAGE_KEY = 'simple-loop-claw-sidebar-hidden-projects'
const PROJECT_ORGANIZATION_STORAGE_KEY = 'simple-loop-claw-sidebar-project-organization'
const PROJECT_SORT_STORAGE_KEY = 'simple-loop-claw-sidebar-project-sort'
...
describe('Sidebar', () => {
  ...
  it('does not restore a hidden Windows drive root when creating a child project session', async () => {
    ...
    useSessionStore.setState({
      sessions: [
        makeSession('child-1', 'Child Session', 'D:\\workspace\\code\\simple-loop-claw', now),
      ],
    })
    useTabStore.setState({
    ...
    await waitFor(() => {
      expect(createSession).toHaveBeenCalledWith('D:\\workspace\\code\\simple-loop-claw')
    })
