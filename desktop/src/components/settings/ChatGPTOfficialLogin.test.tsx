...
const initialOAuthState = useLoopClawOpenAIOAuthStore.getState()
...
describe('ChatGPTOfficialLogin', () => {
  ...
  afterEach(() => {
    act(() => {
      ...
      useLoopClawOpenAIOAuthStore.setState(initialOAuthState)
    })
    vi.useRealTimers()
    cleanup()
    vi.restoreAllMocks()
  ...
  it('keeps an actionable authorization link when shell open fails', async () => {
    ...
    expect(useLoopClawOpenAIOAuthStore.getState().error).toBeNull()
