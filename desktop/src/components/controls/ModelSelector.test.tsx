...
import { useLoopClawOpenAIOAuthStore } from '../../stores/loopClawOpenAIOAuthStore'
...
afterEach(() => {
  ...
  useLoopClawOAuthStore.setState(useLoopClawOAuthStore.getInitialState(), true)
  useLoopClawOpenAIOAuthStore.setState(useLoopClawOpenAIOAuthStore.getInitialState(), true)
})

beforeEach(() => {
  useLoopClawOAuthStore.setState({ fetchStatus: async () => {} })
  useLoopClawOpenAIOAuthStore.setState({ fetchStatus: async () => {} })
})
...
describe('ModelSelector', () => {
  it('does not query official OAuth status when mounted', () => {
    ...
    useLoopClawOAuthStore.setState({ fetchStatus: fetchClaudeStatus })
    useLoopClawOpenAIOAuthStore.setState({ fetchStatus: fetchOpenAIStatus })
  ...
  it('hides official provider sections when OAuth is not logged in', async () => {
    useLoopClawOAuthStore.setState({ status: { loggedIn: false }, fetchStatus: async () => {} })
    useLoopClawOpenAIOAuthStore.setState({ status: { loggedIn: false }, fetchStatus: async () => {} })
    useSettingsStore.setState({
      locale: 'en',
      availableModels: MODELS,
      currentModel: MODELS[0],
