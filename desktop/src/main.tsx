...
declare global {
  interface Window {
    __LOOP_CLAW_BOOTSTRAPPED__?: boolean
    __LOOP_CLAW_SHOW_STARTUP_ERROR__?: (reason: unknown) => void
  }
}
...
export async function bootstrapDesktopApp(
  ...
  try {
    ...
    window.__LOOP_CLAW_BOOTSTRAPPED__ = true
  } catch (error) {
    console.error('[desktop] Failed to bootstrap app', error)
    if (root) {
      if (window.__LOOP_CLAW_SHOW_STARTUP_ERROR__) {
        window.__LOOP_CLAW_SHOW_STARTUP_ERROR__(error)
      } else {
