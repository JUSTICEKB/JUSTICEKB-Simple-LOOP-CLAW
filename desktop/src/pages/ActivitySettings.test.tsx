...
describe('ActivitySettings', () => {
  ...
  it('renders summary metrics and a GitHub-style trailing heatmap without future days', async () => {
    ...
    expect(screen.getByText('simple-loop-claw')).toBeInTheDocument()
    expect(screen.getByAltText('simple-loop-claw avatar')).toHaveAttribute('src', '/app-icon.png')
    expect(screen.getByAltText('simple-loop-claw avatar')).toHaveClass('scale-[1.28]')
    expect(screen.getByRole('link', { name: 'github.com/NanmiCoder/simple-loop-claw' })).toHaveAttribute(
      'href',
      'https://github.com/NanmiCoder/simple-loop-claw',
    )
