...
const traceList: TraceSessionList = {
  total: 1,
  storageDir: '/tmp/simple-loop-claw/traces',
  settings: {
    enabled: true,
    storageDir: '/tmp/simple-loop-claw/traces',
  },
...
describe('TraceList', () => {
  ...
  it('renders rows with title, model chips, failure count and metrics', async () => {
    ...
    expect(screen.getByText('/tmp/simple-loop-claw/traces')).toBeInTheDocument()
  ...
  it('shows the paused badge when capture is disabled', async () => {
    vi.mocked(tracesApi.list).mockResolvedValue({
      ...traceList,
      settings: { enabled: false, storageDir: '/tmp/simple-loop-claw/traces' },
    })
