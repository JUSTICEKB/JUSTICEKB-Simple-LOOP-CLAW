...
export const kr: Record<TranslationKey, string> = {
  ...
  'settings.terminal.description': '플러그인, 스킬, MCP 설정을 위해 호스트 컴퓨터의 명령을 실행합니다. 데스크톱 앱에는 loop-claw가 포함되어 있습니다. 문서의 claude <args>를 loop-claw <args>로 바꾸세요. 예: loop-claw plugin install ... 또는 loop-claw mcp add ...',
  ...
  'settings.diagnostics.doctorSafeKeys': '다음만 지웁니다: simple-loop-claw-open-tabs, simple-loop-claw-session-runtime, simple-loop-claw-theme, simple-loop-claw-locale, simple-loop-claw.persistence.schemaVersion.',
  ...
  'settings.providers.settingsJsonDesc': '~/.claude/simple-loop-claw/settings.json — 직접 편집할 수 있습니다. 저장 시 기록됩니다.',
  ...
  'settings.general.storagePortableDirPlaceholder': 'simple-loop-claw 데이터용 폴더 선택',
  ...
  'settings.general.storageMoveHint': '디렉터리를 전환해도 기존 데이터는 마이그레이션되지 않습니다. 이전 세션을 계속 표시하려면 이전 디렉터리에서 projects, skills, plugins, simple-loop-claw 및 관련 폴더를 복사하세요. 포터블 번들의 경우 이 폴더를 앱 옆에 두고 함께 압축하세요.',
  ...
  'settings.general.notificationsTestTitle': 'Simple LoopClaw 알림이 사용 설정되었습니다',
  ...
  'settings.general.traceHintOn': '새 세션은 간결한 요청, 응답, 상태 이벤트를 로컬 simple-loop-claw traces 디렉터리에 기록합니다.',
