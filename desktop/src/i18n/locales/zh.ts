...
export const zh: Record<TranslationKey, string> = {
  ...
  'settings.terminal.description': '直接运行宿主机命令，用于安装插件、Skills、MCP 等扩展。桌面端已内置 loop-claw；文档里的 claude <参数> 可替换成 loop-claw <参数>，例如 loop-claw plugin install ... 或 loop-claw mcp add ...',
  ...
  'settings.diagnostics.doctorSafeKeys': '只会清理：simple-loop-claw-open-tabs、simple-loop-claw-session-runtime、simple-loop-claw-theme、simple-loop-claw-locale 和 simple-loop-claw.persistence.schemaVersion。',
  ...
  'settings.providers.settingsJsonDesc': '~/.claude/simple-loop-claw/settings.json — 直接编辑，保存时写入。',
  ...
  'settings.general.storagePortableDirPlaceholder': '选择一个用于保存 simple-loop-claw 数据的文件夹',
  ...
  'settings.general.storageMoveHint': '切换目录不会自动搬迁旧数据。如果希望旧会话继续出现，请把原目录下的 projects、skills、plugins、simple-loop-claw 等数据复制到新目录。要做便携包，建议把目录放在应用旁边并一起压缩。',
  ...
  'settings.general.notificationsTestTitle': 'Simple LoopClaw 通知已启用',
  ...
  'settings.general.traceHintOn': '新会话会把精简请求、响应、状态事件写入本机 simple-loop-claw traces 目录。',
