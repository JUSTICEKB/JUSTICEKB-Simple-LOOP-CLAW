...
export const zh: Record<TranslationKey, string> = {
  ...
  'settings.terminal.description': '直接執行宿主機命令，用於安裝外掛、Skills、MCP 等擴充套件。桌面端已內建 loop-claw；文件裡的 claude <引數> 可替換成 loop-claw <引數>，例如 loop-claw plugin install ... 或 loop-claw mcp add ...',
  ...
  'settings.diagnostics.doctorSafeKeys': '只會清理：simple-loop-claw-open-tabs、simple-loop-claw-session-runtime、simple-loop-claw-theme、simple-loop-claw-locale 和 simple-loop-claw.persistence.schemaVersion。',
  ...
  'settings.providers.settingsJsonDesc': '~/.claude/simple-loop-claw/settings.json — 直接編輯，儲存時寫入。',
  ...
  'settings.general.storagePortableDirPlaceholder': '選擇一個用於儲存 simple-loop-claw 資料的資料夾',
  ...
  'settings.general.storageMoveHint': '切換目錄不會自動搬遷舊資料。如果希望舊會話繼續出現，請把原目錄下的 projects、skills、plugins、simple-loop-claw 等資料複製到新目錄。要做便攜包，建議把目錄放在應用旁邊並一起壓縮。',
  ...
  'settings.general.notificationsTestTitle': 'Simple LoopClaw 通知已啟用',
  ...
  'settings.general.traceHintOn': '新會話會把精簡請求、回應、狀態事件寫入本機 simple-loop-claw traces 目錄。',
