...
export const jp: Record<TranslationKey, string> = {
  ...
  'settings.terminal.description': 'プラグイン、スキル、MCP のセットアップ用にホストマシンのコマンドを実行します。デスクトップアプリには loop-claw が含まれています。ドキュメントの claude <args> を loop-claw <args> に置き換えてください。例: loop-claw plugin install ... や loop-claw mcp add ...',
  ...
  'settings.diagnostics.doctorSafeKeys': 'クリアされるのは次のみ: simple-loop-claw-open-tabs、simple-loop-claw-session-runtime、simple-loop-claw-theme、simple-loop-claw-locale、simple-loop-claw.persistence.schemaVersion。',
  ...
  'settings.providers.settingsJsonDesc': '~/.claude/simple-loop-claw/settings.json — 直接編集できます。保存時に書き込まれます。',
  ...
  'settings.general.storagePortableDirPlaceholder': 'simple-loop-claw データ用のフォルダを選択',
  ...
  'settings.general.storageMoveHint': 'ディレクトリを切り替えても既存のデータは移行されません。以前のセッションを引き続き表示したい場合は、古いディレクトリから projects、skills、plugins、simple-loop-claw および関連フォルダをコピーしてください。ポータブルバンドルにする場合は、このフォルダをアプリの隣に置いて一緒に zip 化してください。',
  ...
  'settings.general.notificationsTestTitle': 'Simple LoopClaw の通知が有効になりました',
  ...
  'settings.general.traceHintOn': '新しいセッションは、簡潔な要求、応答、状態イベントをローカルの simple-loop-claw traces ディレクトリに書き込みます。',
