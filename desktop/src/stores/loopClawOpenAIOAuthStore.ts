// desktop/src/stores/loopClawOpenAIOAuthStore.ts
...
export const useLoopClawOpenAIOAuthStore = create<LoopClawOpenAIOAuthState>((set, get) => {
  ...
  return {
    ...
    fetchStatus: async () => {
      try {
        const status = await loopClawOpenAIOAuthApi.status()
    ...
    login: async () => {
      ...
      try {
        const res = await loopClawOpenAIOAuthApi.start()
        set({ isLoading: false })
    ...
    logout: async () => {
      ...
      set({ isLoading: true, error: null })
      try {
        await loopClawOpenAIOAuthApi.logout()
