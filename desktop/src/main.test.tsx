...
describe('desktop bootstrap', () => {
  afterEach(() => {
    ...
    delete window.__LOOP_CLAW_BOOTSTRAPPED__
    delete window.__LOOP_CLAW_SHOW_STARTUP_ERROR__
    vi.restoreAllMocks()
    vi.clearAllMocks()
  })
  ...
  it('runs startup migrations and renders the app without top-level await', async () => {
    ...
    expect(window.__LOOP_CLAW_BOOTSTRAPPED__).toBe(true)
  })
  ...
  it('delegates bootstrap failures to the HTML startup watchdog when available', async () => {
    ...
    const consoleError = vi.spyOn(console, 'error').mockImplementation(() => {})
    window.__LOOP_CLAW_SHOW_STARTUP_ERROR__ = showStartupError
