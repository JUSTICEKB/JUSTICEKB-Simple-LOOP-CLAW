...
SESSION_NAME="simple-loop-claw-install-e2e-${RUN_ID}"
ARTIFACT_DIR="$(mktemp -d "/tmp/simple-loop-claw-install-e2e-${RUN_ID}-XXXX")"
SERVER_LOG="${ARTIFACT_DIR}/server.log"
WEB_LOG="${ARTIFACT_DIR}/web.log"

if [[ "${LOOP_CLAW_E2E_USE_REAL_CONFIG:-1}" == "1" ]]; then
