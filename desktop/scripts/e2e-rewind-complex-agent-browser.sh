...
SESSION_NAME="simple-loop-claw-rewind-complex-e2e-${RUN_ID}"
ARTIFACT_DIR="$(mktemp -d "/tmp/simple-loop-claw-rewind-complex-e2e-${RUN_ID}-XXXX")"
...
${AB} open "${WEB_URL}"
${AB} wait 2000
${AB} eval "localStorage.setItem('simple-loop-claw-locale', 'en'); location.reload();"
${AB} wait 2000
