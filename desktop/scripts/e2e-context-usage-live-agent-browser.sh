...
SESSION_NAME="simple-loop-claw-context-live-${RUN_ID}"
ARTIFACT_DIR="${ARTIFACT_DIR:-$(mktemp -d "/tmp/simple-loop-claw-context-live-${RUN_ID}-XXXX")}"
...
"${AB[@]}" eval "localStorage.setItem('simple-loop-claw-locale', 'en'); localStorage.setItem('simple-loop-claw-open-tabs', '${TAB_STATE}'); location.reload();" >>"${BROWSER_LOG}" 2>&1
