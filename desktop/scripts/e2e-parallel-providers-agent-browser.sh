...
ARTIFACT_DIR="${ARTIFACT_DIR:-/tmp/simple-loop-claw-e2e-parallel-providers-$$}"
CONFIG_DIR="${ARTIFACT_DIR}/config"
PROJECT_A="${ARTIFACT_DIR}/project-a"
PROJECT_B="${ARTIFACT_DIR}/project-b"
SESSION_NAME_A="${SESSION_NAME_A:-simple-loop-claw-provider-a-$$}"
SESSION_NAME_B="${SESSION_NAME_B:-simple-loop-claw-provider-b-$$}"
...
mkdir -p "${ARTIFACT_DIR}" "${CONFIG_DIR}/simple-loop-claw" "${PROJECT_A}" "${PROJECT_B}"
...
mkdirSync(join(configDir, 'simple-loop-claw'), { recursive: true })
writeFileSync(join(configDir, 'simple-loop-claw', 'providers.json'), JSON.stringify({
  schemaVersion: 1,
...
setup_browser() {
  ...
  "${ab[@]}" eval "
    localStorage.setItem('simple-loop-claw-open-tabs', JSON.stringify({
      openTabs: [{ sessionId: '${session_id}', title: '${title}', type: 'session' }],
      activeTabId: '${session_id}',
    }));
    localStorage.setItem('simple-loop-claw-session-runtime', JSON.stringify({
