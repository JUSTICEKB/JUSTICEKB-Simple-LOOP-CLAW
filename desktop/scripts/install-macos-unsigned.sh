...
#
# Installs the unsigned macOS build of Simple LoopClaw from a DMG that sits
...
main() {
  ...
  osascript -e 'quit app "Simple LoopClaw"' >/dev/null 2>&1 || true

  if [ -d "$APP_PATH" ]; then
    local backup
    backup="${HOME}/.Trash/Simple LoopClaw.$(date +%Y%m%d%H%M%S).app"
