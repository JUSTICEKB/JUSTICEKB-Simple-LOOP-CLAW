...
- 新增 LoopClaw OpenAI OAuth API 和桌面 store，登录状态不会回传 token 本体，回调路径统一走桌面 server，并兼容旧 `/callback/openai` 路径。
...
- OpenAI OAuth token 存储统一到 `~/.claude/simple-loop-claw/openai-oauth.json`，并对 refresh token、id token、client id、过期刷新和原子写入做了更完整处理。
