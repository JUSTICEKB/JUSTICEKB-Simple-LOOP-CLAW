...
- 如果本版本仍未完成 Apple / Windows 代码签名，首次安装时系统可能需要手动放行一次（见下方安装说明）。覆盖安装不会删除本地数据（`~/.claude`、`~/.claude/simple-loop-claw` 及自定义便携数据目录）。
...
| 平台 | 文件 |
|------|------|
| macOS Apple Silicon（M 系列） | `Claude-Code-LoopClaw-0.4.2-mac-arm64.dmg` |
| macOS Intel | `Claude-Code-LoopClaw-0.4.2-mac-x64.dmg` |
| Windows x64 | `Claude-Code-LoopClaw-0.4.2-win-x64.exe` |
| Linux x64 | `Claude-Code-LoopClaw-0.4.2-linux-x86_64.AppImage` 或 `Claude-Code-LoopClaw-0.4.2-linux-amd64.deb` |
| Linux ARM64 | `Claude-Code-LoopClaw-0.4.2-linux-arm64.AppImage` 或 `Claude-Code-LoopClaw-0.4.2-linux-arm64.deb` |
...
```bash
chmod +x Claude-Code-LoopClaw-0.4.2-linux-x86_64.AppImage
./Claude-Code-LoopClaw-0.4.2-linux-x86_64.AppImage
```

deb：

```bash
sudo apt install ./Claude-Code-LoopClaw-0.4.2-linux-amd64.deb
```
