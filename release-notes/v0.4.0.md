...
- 暂不提供在线自动更新：在拿到 Apple 签名前，请每次到 [GitHub Releases](https://github.com/NanmiCoder/simple-loop-claw/releases) 手动下载新版本覆盖安装。覆盖安装不会删除本地数据（`~/.claude`、`~/.claude/simple-loop-claw` 及自定义便携数据目录）。
...
| 平台 | 文件 |
|------|------|
| macOS Apple Silicon（M 系列） | `Claude-Code-LoopClaw-0.4.0-mac-arm64.dmg` |
| macOS Intel | `Claude-Code-LoopClaw-0.4.0-mac-x64.dmg` |
| Windows x64 | `Claude-Code-LoopClaw-0.4.0-win-x64.exe` |
| Linux x64 | `Claude-Code-LoopClaw-0.4.0-linux-x86_64.AppImage` 或 `Claude-Code-LoopClaw-0.4.0-linux-amd64.deb` |
| Linux ARM64 | `Claude-Code-LoopClaw-0.4.0-linux-arm64.AppImage` 或 `Claude-Code-LoopClaw-0.4.0-linux-arm64.deb` |
...
```bash
chmod +x Claude-Code-LoopClaw-0.4.0-linux-x86_64.AppImage
./Claude-Code-LoopClaw-0.4.0-linux-x86_64.AppImage
```

deb：

```bash
sudo apt install ./Claude-Code-LoopClaw-0.4.0-linux-amd64.deb
```
