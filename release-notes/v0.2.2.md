...
- 如果升级后遇到应用无法启动或持续白屏，请优先使用桌面端 Doctor/恢复入口处理可再生的本地 UI 状态；不要手动删除 `~/.claude` 或 `~/.claude/simple-loop-claw/` 目录，避免误删 Provider、OAuth、插件、技能或会话相关配置。
