...
- 桌面侧边栏按项目 root 分组，避免 worktree 会话散落成多个路径；项目排序、隐藏、置顶和组织偏好持久化到 simple-loop-claw 自有配置，并兼容 H5。
...
- 修复 legacy provider 配置迁移时缺乏 simple-loop-claw 专属落点的问题；迁移会导入到 `~/.claude/simple-loop-claw`，不会删除原始共享配置。
...
- 本版本包含较多桌面状态和本地持久化迁移：升级后如遇到启动异常，请优先保留诊断页内容反馈，不要删除 `~/.claude` 或 `~/.claude/simple-loop-claw`。
