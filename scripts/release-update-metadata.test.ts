...
function tempDir() {
  return mkdtempSync(join(tmpdir(), 'simple-loop-claw-release-metadata-'))
}
...
describe('release update metadata merge', () => {
  test('merges namespaced macOS x64 and arm64 metadata into standard latest-mac.yml', () => {
    ...
    writeYaml(join(inputDir, 'latest-mac-macOS-ARM64.yml'), `
      version: 0.3.2
      files:
        - url: Claude-Code-LoopClaw-0.3.2-arm64.zip
          sha512: arm64-checksum
          size: 222
      path: Claude-Code-LoopClaw-0.3.2-arm64.zip
      sha512: arm64-checksum
    ...
    writeYaml(join(inputDir, 'latest-mac-macOS-x64.yml'), `
      version: 0.3.2
      files:
        - url: Claude-Code-LoopClaw-0.3.2-x64.zip
          sha512: x64-checksum
          size: 111
      path: Claude-Code-LoopClaw-0.3.2-x64.zip
      sha512: x64-checksum
    ...
    expect(merged.files.map(file => file.url)).toEqual([
      'Claude-Code-LoopClaw-0.3.2-x64.zip',
      'Claude-Code-LoopClaw-0.3.2-arm64.zip',
    ])
    expect(merged.path).toBe('Claude-Code-LoopClaw-0.3.2-x64.zip')
  ...
  test('keeps macOS zip metadata primary when each architecture has dmg and zip files', () => {
    ...
    writeYaml(join(inputDir, 'latest-mac-macOS-ARM64.yml'), `
      version: 0.3.2
      files:
        - url: Claude-Code-LoopClaw-0.3.2-arm64.dmg
    ...
    writeYaml(join(inputDir, 'latest-mac-macOS-x64.yml'), `
      version: 0.3.2
      files:
        - url: Claude-Code-LoopClaw-0.3.2-x64.dmg
    ...
    expect(merged.files.map(file => file.url)).toEqual([
      'Claude-Code-LoopClaw-0.3.2-x64.zip',
      'Claude-Code-LoopClaw-0.3.2-x64.dmg',
      'Claude-Code-LoopClaw-0.3.2-arm64.zip',
      'Claude-Code-LoopClaw-0.3.2-arm64.dmg',
    ])
    expect(merged.path).toBe('Claude-Code-LoopClaw-0.3.2-x64.zip')
  ...
  test('restores standard linux channel names for x64 and arm64 metadata', () => {
    ...
    writeYaml(join(inputDir, 'latest-linux-Linux-x64.yml'), `
      version: 0.3.2
      files:
        - url: Claude-Code-LoopClaw-0.3.2-x64.AppImage
          sha512: linux-x64-checksum
          size: 111
      path: Claude-Code-LoopClaw-0.3.2-x64.AppImage
      sha512: linux-x64-checksum
    `)
    writeYaml(join(inputDir, 'latest-linux-Linux-ARM64.yml'), `
      version: 0.3.2
      files:
        - url: Claude-Code-LoopClaw-0.3.2-arm64.AppImage
          sha512: linux-arm64-checksum
          size: 222
      path: Claude-Code-LoopClaw-0.3.2-arm64.AppImage
      sha512: linux-arm64-checksum
    `)
    ...
    expect(x64.path).toBe('Claude-Code-LoopClaw-0.3.2-x64.AppImage')
    expect(arm64.path).toBe('Claude-Code-LoopClaw-0.3.2-arm64.AppImage')
  })
  ...
  test('keeps Linux AppImage as primary update artifact when deb is also published', () => {
    ...
    writeYaml(join(inputDir, 'latest-linux-Linux-x64.yml'), `
      version: 0.3.2
      files:
        - url: Claude-Code-LoopClaw-0.3.2-linux-amd64.deb
          sha512: linux-deb-checksum
          size: 222
        - url: Claude-Code-LoopClaw-0.3.2-linux-x86_64.AppImage
          sha512: linux-appimage-checksum
          size: 111
      path: Claude-Code-LoopClaw-0.3.2-linux-amd64.deb
      sha512: linux-deb-checksum
    `)
    ...
    expect(x64.files.map(file => file.url)).toEqual([
      'Claude-Code-LoopClaw-0.3.2-linux-x86_64.AppImage',
      'Claude-Code-LoopClaw-0.3.2-linux-amd64.deb',
    ])
    expect(x64.path).toBe('Claude-Code-LoopClaw-0.3.2-linux-x86_64.AppImage')
  ...
  test('restores standard Windows channel metadata after matrix namespacing', () => {
    ...
    writeYaml(join(inputDir, 'latest-Windows-x64.yml'), `
      version: 0.3.2
      files:
        - url: Claude-Code-LoopClaw-0.3.2-win-x64.exe
          sha512: win-checksum
          size: 333
      path: Claude-Code-LoopClaw-0.3.2-win-x64.exe
      sha512: win-checksum
    `)
    writeYaml(join(inputDir, 'latest-Windows-ARM64.yml'), `
      version: 0.3.2
      files:
        - url: Claude-Code-LoopClaw-0.3.2-win-arm64.exe
          sha512: win-arm64-checksum
          size: 222
      path: Claude-Code-LoopClaw-0.3.2-win-arm64.exe
      sha512: win-arm64-checksum
    `)
    ...
    expect(windows.files.map(file => file.url)).toEqual([
      'Claude-Code-LoopClaw-0.3.2-win-x64.exe',
      'Claude-Code-LoopClaw-0.3.2-win-arm64.exe',
    ])
    expect(windows.path).toBe('Claude-Code-LoopClaw-0.3.2-win-x64.exe')
  ...
  test('rejects metadata groups with mixed app versions', () => {
    ...
    writeYaml(join(inputDir, 'latest-mac-macOS-ARM64.yml'), `
      version: 0.3.3
      files:
        - url: Claude-Code-LoopClaw-0.3.3-arm64.zip
          sha512: arm64-checksum
    `)
