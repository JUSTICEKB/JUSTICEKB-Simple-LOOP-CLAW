...
export async function executeDesktopSmoke(
  ...
  try {
    ...
    const browserSetup = [
      `localStorage.setItem('simple-loop-claw-open-tabs', ${JSON.stringify(JSON.stringify({
        openTabs: [{ sessionId: session.sessionId, title: 'Desktop Smoke', type: 'session' }],
        activeTabId: session.sessionId,
      }))})`,
      runtimeSelection
        ? `localStorage.setItem('simple-loop-claw-session-runtime', ${JSON.stringify(JSON.stringify({
          [session.sessionId]: runtimeSelection,
        }))})`
        : `localStorage.removeItem('simple-loop-claw-session-runtime')`,
    ]
