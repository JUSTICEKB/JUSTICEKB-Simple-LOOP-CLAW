...
describe('Feishu: project picker card', () => {
  const sampleProjects: RecentProject[] = [
    {
      projectPath: '/Users/dev/simple-loop-claw',
      realPath: '/Users/dev/simple-loop-claw',
      projectName: 'simple-loop-claw',
      isGit: true,
      repoName: 'simple-loop-claw',
      branch: 'main',
  ...
  it('info column has title markdown + notation path markdown', () => {
    ...
    expect(info[0].content).toContain('**simple-loop-claw**')
    ...
    expect(info[1].content).toContain('simple-loop-claw')
  })
  ...
  it('row button says 选择 with small size and carries per-project value', () => {
    ...
    expect(btn1.value.realPath).toBe('/Users/dev/simple-loop-claw')
    expect(btn1.value.projectName).toBe('simple-loop-claw')
...
describe('Feishu: card.action.trigger parsing', () => {
  ...
  it('parses pick_project action from event', () => {
    ...
    expect(event.action.value.realPath).toBe('/Users/dev/simple-loop-claw')
    expect(event.action.value.projectName).toBe('simple-loop-claw')
  })
