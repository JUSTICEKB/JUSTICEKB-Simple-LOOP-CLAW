...
describe('DingTalk AI Card streaming', () => {
  ...
  it('times out a hung card streaming request', async () => {
    const previousTimeout = process.env.LOOP_CLAW_IM_CARD_REQUEST_TIMEOUT_MS
    process.env.LOOP_CLAW_IM_CARD_REQUEST_TIMEOUT_MS = '20'
    globalThis.fetch = mock(async (url: string | URL | Request, init?: RequestInit) => {
      calls.push({
        url: String(url),
        method: init?.method ?? 'GET',
    ...
    try {
      ...
      if (previousTimeout === undefined) {
        delete process.env.LOOP_CLAW_IM_CARD_REQUEST_TIMEOUT_MS
      } else {
        process.env.LOOP_CLAW_IM_CARD_REQUEST_TIMEOUT_MS = previousTimeout
      }
    }
  })
})
