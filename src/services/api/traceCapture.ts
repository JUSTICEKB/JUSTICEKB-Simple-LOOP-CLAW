...
export function shouldCaptureApiTrace(): boolean {
  if (isEnvDefinedFalsy(process.env.LOOP_CLAW_TRACE_API_CALLS)) return false
  if (isEnvTruthy(process.env.LOOP_CLAW_TRACE_API_CALLS)) return true
...
export function getTraceStorageDir(): string {
  return join(getClaudeConfigHomeDir(), 'simple-loop-claw', 'traces')
}
...
function getManagedSettingsPath(): string {
  return join(getClaudeConfigHomeDir(), 'simple-loop-claw', 'settings.json')
}
