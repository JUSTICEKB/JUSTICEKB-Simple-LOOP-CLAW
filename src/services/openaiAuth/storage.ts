...
function getLoopClawDir(): string {
  const configDir =
    process.env.CLAUDE_CONFIG_DIR || path.join(os.homedir(), '.claude')
  return path.join(configDir, 'simple-loop-claw')
}

function getFileBackedStorageMarkerPath(): string {
  return path.join(getLoopClawDir(), FILE_BACKED_STORAGE_MARKER_FILE)
}

function markFileBackedStorageUsed(): void {
  try {
    fs.mkdirSync(getLoopClawDir(), { recursive: true })
