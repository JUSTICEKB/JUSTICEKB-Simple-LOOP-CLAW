...
export class ConversationService {
  ...
  private async buildChildEnv(
    ...
    // from ~/.claude/simple-loop-claw/settings.json instead of stale process.env.
    //
    ...
    const PROVIDER_ENV_KEYS = [
      ...
      'LOOP_CLAW_SEND_DISABLED_THINKING',
    ...
    delete cleanEnv.LOOP_CLAW_TRACE_PROVIDER_ID
    delete cleanEnv.LOOP_CLAW_TRACE_PROVIDER_NAME
    delete cleanEnv.LOOP_CLAW_TRACE_PROVIDER_FORMAT
    ...
    return {
      ...
      ...(sdkUrl && traceCaptureEnabled && explicitProvider
        ? {
            LOOP_CLAW_TRACE_PROVIDER_ID: explicitProvider.id,
            LOOP_CLAW_TRACE_PROVIDER_NAME: explicitProvider.name,
            LOOP_CLAW_TRACE_PROVIDER_FORMAT: explicitProvider.apiFormat ?? 'anthropic',
          }
        : {}),
      ...(desktopServerUrl
        ? { LOOP_CLAW_DESKTOP_SERVER_URL: desktopServerUrl }
        : {}),
      ...(sdkUrl
        ? {
            LOOP_CLAW_DESKTOP_AWAIT_MCP: '1',
            LOOP_CLAW_DESKTOP_AWAIT_MCP_TIMEOUT_MS: '5000',
          }
        : {}),
      ...
      LOOP_CLAW_SKIP_DOTENV: '1',
      ...(explicitProviderEnv
        ? { CLAUDE_CODE_PROVIDER_MANAGED_BY_HOST: '1' }
        : {}),
      // "官方" 模式 (simple-loop-claw/settings.json 没 provider env) 下,把 CLI 标记为
  ...
  private async buildOfficialOAuthEnv(): Promise<Record<string, string>> {
    ...
    try {
      ...
      const { loopClawOAuthService } = await import('./loopClawOAuthService.js')
      const token = await loopClawOAuthService.ensureFreshAccessToken()
      if (token) {
  ...
  private async refreshOfficialOAuthTokenBeforeTurn(
    ...
    try {
      const { loopClawOAuthService } = await import('./loopClawOAuthService.js')
      token = await loopClawOAuthService.ensureFreshAccessToken()
    } catch (err) {
      console.error(
   ...
   * 强制切 OAuth。
   *
   * 默认 (读不到 settings.json) 按"官方"处理 — 即使用户从未用过 simple-loop-claw
  ...
  private shouldMarkManagedOAuth(providerId?: string | null): boolean {
    ...
    const configDir =
      process.env.CLAUDE_CONFIG_DIR || path.join(os.homedir(), '.claude')
    const settingsPath = path.join(configDir, 'simple-loop-claw', 'settings.json')
    try {
      const raw = fs.readFileSync(settingsPath, 'utf-8')
      const parsed = JSON.parse(raw) as { env?: Record<string, string> }
      const env = parsed.env ?? {}
  ...
  private resolveCliArgs(baseArgs: string[]): string[] {
    ...
    if (!launcher) {
      ...
      return [path.resolve(import.meta.dir, '../../../bin/loop-claw'), ...baseArgs]
    }
  ...
  private buildStartupError(
    ...
    if (
      ...
      return new ConversationStartupError(
        'Desktop chat could not start because Claude CLI is not authenticated. Run `./bin/loop-claw /login` or provide valid API credentials, then retry.',
        'CLI_AUTH_REQUIRED',
      )
    }
