 ...
 *
 * Storage: ~/.claude/simple-loop-claw/providers.json (lightweight index)
 * Active provider env vars written to ~/.claude/simple-loop-claw/settings.json
...
import { loopClawOpenAIOAuthService } from './loopClawOpenAIOAuthService.js'
import {
...
export class ProviderService {
   ...
   *  1. A simple-loop-claw provider is active → has auth
  ...
  async checkAuthStatus(): Promise<{
    hasAuth: boolean
    source: 'simple-loop-claw-provider' | 'openai-oauth' | 'original-settings' | 'env' | 'none'
    activeProvider?: string
  }> {
    // 1. Check simple-loop-claw active provider
    const index = await this.readIndex()
    if (index.activeId) {
      ...
      if (provider) {
        ...
        if (Object.values(authEnv).some(value => value.length > 0)) {
          return { hasAuth: true, source: 'simple-loop-claw-provider', activeProvider: provider.name }
        }
      }
    }
