...
async function defaultConvertIconToPng(iconPath: string, size: number): Promise<Uint8Array> {
  ...
  const tmpDir = await mkdtemp(join(tmpdir(), 'simple-loop-claw-open-target-icon-'))
...
$source = $env:LOOP_CLAW_ICON_SOURCE
$output = $env:LOOP_CLAW_ICON_OUTPUT
...
`
  ...
  await execFile('powershell.exe', [
    ...
    env: {
      ...process.env,
      LOOP_CLAW_ICON_SOURCE: iconPath,
      LOOP_CLAW_ICON_OUTPUT: outputPath,
    },
    timeout: 5_000,
    windowsHide: true,
  })
