...
export class DiagnosticsService {
  ...
  getLogDir(): string {
    return path.join(this.getConfigDir(), 'simple-loop-claw', 'diagnostics')
  }
  ...
  async recordEvent(input: DiagnosticEventInput): Promise<void> {
    ...
    // ~/.claude/simple-loop-claw/diagnostics. Tests that genuinely exercise diagnostics
  ...
  async exportBundle(): Promise<DiagnosticsExportResult> {
    ...
    const fileName = `simple-loop-claw-diagnostics-${timestamp}.tar.gz`
  ...
  private buildReadme(): string {
    return [
      'simple-loop-claw diagnostics bundle',
      '',
  ...
  private buildRecentErrorsSummary(events: DiagnosticEvent[]): string {
    ...
    const lines = [
      '# simple-loop-claw recent diagnostics',
      '',
  ...
  private async buildProvidersSummary(): Promise<Record<string, unknown>> {
    const providerPath = path.join(this.getConfigDir(), 'simple-loop-claw', 'providers.json')
    try {
