...
async function migrateLegacyRootProviders(
  ...
  try {
    ...
    if (errnoCode(error) !== 'ENOENT') {
      report.failures.push(`simple-loop-claw/providers.json: ${error instanceof Error ? error.message : String(error)}`)
      return
    }
  }
  ...
  try {
    ...
    if (managedSettings) {
      ...
      report.migratedEntries.push('providers.json -> simple-loop-claw/settings.json')
    }
  } catch (error) {
    if (error instanceof SyntaxError) {
