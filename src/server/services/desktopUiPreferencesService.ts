...
const DEFAULT_PROFILE_SUBTITLE = 'github.com/NanmiCoder/simple-loop-claw'
...
export class DesktopUiPreferencesService {
  ...
  private getPreferencesPath(): string {
    return path.join(this.getConfigDir(), 'simple-loop-claw', 'desktop-ui.json')
  }

  private getProfileDir(): string {
    return path.join(this.getConfigDir(), 'simple-loop-claw', 'profile')
  }
  ...
  private getProfileAvatarPath(avatarFile: string): string {
    ...
    return path.join(this.getConfigDir(), 'simple-loop-claw', normalized)
  }
  ...
  async readPreferences(): Promise<DesktopUiPreferencesReadResult> {
    ...
    const preferences = await readRecoverableJsonFile({
      filePath,
      label: 'simple-loop-claw desktop UI preferences',
