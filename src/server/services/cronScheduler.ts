...
export function resolveCronTaskTimeoutMs(
  env: { LOOP_CLAW_TASK_TIMEOUT_MS?: string } = process.env,
): number {
  const raw = env.LOOP_CLAW_TASK_TIMEOUT_MS?.trim()
...
export function resolveCronProjectRoot(
  ...
  const explicitRoot = env.LOOP_CLAW_ROOT?.trim()
  ...
  private shouldMarkManagedOAuth(providerId?: string | null): boolean {
    ...
    try {
      const raw = readFileSync(
        path.join(this.getConfigDir(), 'simple-loop-claw', 'settings.json'),
        'utf-8',
      )
      const parsed = JSON.parse(raw) as { env?: Record<string, string> }
      const env = parsed.env ?? {}
