...
const DESKTOP_CLI_NAME = 'loop-claw'
const DESKTOP_CLI_WINDOWS_LEGACY_EXE = `${DESKTOP_CLI_NAME}.exe`
const PATH_BLOCK_START = '# >>> Simple LoopClaw PATH >>>'
const PATH_BLOCK_END = '# <<< Simple LoopClaw PATH <<<'
...
if [[ ! -x "$SIDECAR" ]]; then
  echo "loop-claw launcher could not find bundled sidecar: $SIDECAR" >&2
  exit 127
fi
...
export function buildWindowsLauncherWrapper(sourcePath: string) {
  ...
  return [
    ...
    '  echo loop-claw launcher could not find bundled sidecar: %SIDECAR% 1>&2',
    '  exit /b 127',
    ')',
