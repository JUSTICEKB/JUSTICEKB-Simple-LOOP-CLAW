...
async function readDesktopUiFile(): Promise<Record<string, unknown>> {
  const raw = await fs.readFile(path.join(tmpDir, 'simple-loop-claw', 'desktop-ui.json'), 'utf-8')
...
describe('DesktopUiPreferencesService', () => {
  ...
  test('returns defaults when desktop-ui.json does not exist', async () => {
    ...
    expect(result.preferences).toEqual({
      schemaVersion: 2,
      profile: {
        displayName: 'simple-loop-claw',
        subtitle: 'github.com/NanmiCoder/simple-loop-claw',
        avatarFile: null,
  ...
  test('normalizes old schema files and preserves unknown fields when updating sidebar preferences', async () => {
    ...
    await fs.writeFile(
      path.join(tmpDir, 'simple-loop-claw', 'desktop-ui.json'),
      JSON.stringify({
  ...
  test('quarantines corrupt desktop-ui.json and reports defaults as missing', async () => {
    await fs.mkdir(path.join(tmpDir, 'simple-loop-claw'), { recursive: true })
    await fs.writeFile(path.join(tmpDir, 'simple-loop-claw', 'desktop-ui.json'), '{bad json', 'utf-8')

    const service = new DesktopUiPreferencesService()
    const result = await service.readPreferences()
    const files = await fs.readdir(path.join(tmpDir, 'simple-loop-claw'))

    expect(result.exists).toBe(false)
    expect(result.preferences.sidebar.hiddenProjects).toEqual([])
    expect(result.preferences.profile.displayName).toBe('simple-loop-claw')
  ...
  test('stores uploaded profile avatars under simple-loop-claw profile storage', async () => {
    const service = new DesktopUiPreferencesService()
    ...
    const avatar = await fs.readFile(path.join(tmpDir, 'simple-loop-claw', 'profile', 'avatar.png'))
  ...
  test('clears only managed profile avatar files', async () => {
    ...
    await fs.writeFile(path.join(tmpDir, 'simple-loop-claw', 'profile', 'local-note.txt'), 'keep me', 'utf-8')
    ...
    await expect(fs.readFile(path.join(tmpDir, 'simple-loop-claw', 'profile', 'avatar.png'))).rejects.toThrow()
    await expect(fs.readFile(path.join(tmpDir, 'simple-loop-claw', 'profile', 'local-note.txt'), 'utf-8')).resolves.toBe('keep me')
  })
...
describe('desktop UI preferences API', () => {
  ...
  test('persists sidebar preferences under simple-loop-claw desktop-ui.json', async () => {
