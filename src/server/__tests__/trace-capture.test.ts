...
describe('trace capture service', () => {
  ...
  test('skips malformed trace jsonl entries when reading a session', async () => {
    const traceDir = path.join(tmpDir, 'simple-loop-claw', 'traces')
  ...
  test('respects managed trace capture settings before writing new records', async () => {
    ...
    const settingsFile = JSON.parse(await fs.readFile(path.join(tmpDir, 'simple-loop-claw', 'settings.json'), 'utf-8')) as {
  ...
  test('captures direct Anthropic-compatible provider calls from desktop fetch override', async () => {
    ...
    const originalTraceEnv = process.env.LOOP_CLAW_TRACE_API_CALLS
    const originalProviderId = process.env.LOOP_CLAW_TRACE_PROVIDER_ID
    const originalProviderName = process.env.LOOP_CLAW_TRACE_PROVIDER_NAME
    const originalProviderFormat = process.env.LOOP_CLAW_TRACE_PROVIDER_FORMAT
    process.env.LOOP_CLAW_TRACE_API_CALLS = '1'
    process.env.LOOP_CLAW_TRACE_PROVIDER_ID = 'provider-sub2api'
    process.env.LOOP_CLAW_TRACE_PROVIDER_NAME = 'Sub2API-ChatGPT'
    process.env.LOOP_CLAW_TRACE_PROVIDER_FORMAT = 'anthropic'
    try {
      ...
      if (originalTraceEnv === undefined) delete process.env.LOOP_CLAW_TRACE_API_CALLS
      else process.env.LOOP_CLAW_TRACE_API_CALLS = originalTraceEnv
      if (originalProviderId === undefined) delete process.env.LOOP_CLAW_TRACE_PROVIDER_ID
      else process.env.LOOP_CLAW_TRACE_PROVIDER_ID = originalProviderId
      if (originalProviderName === undefined) delete process.env.LOOP_CLAW_TRACE_PROVIDER_NAME
      else process.env.LOOP_CLAW_TRACE_PROVIDER_NAME = originalProviderName
      if (originalProviderFormat === undefined) delete process.env.LOOP_CLAW_TRACE_PROVIDER_FORMAT
      else process.env.LOOP_CLAW_TRACE_PROVIDER_FORMAT = originalProviderFormat
    }
  })
...
describe('session trace API', () => {
  ...
  test('lists trace sessions with storage metadata and managed settings', async () => {
    ...
    expect(body.storageDir).toBe(path.join(tmpDir, 'simple-loop-claw', 'traces'))
    expect(body.settings).toEqual({
      enabled: true,
      storageDir: path.join(tmpDir, 'simple-loop-claw', 'traces'),
    })
  })
  ...
  test('deletes a trace session file and invalidates cached reads', async () => {
    ...
    await expect(fs.stat(path.join(tmpDir, 'simple-loop-claw', 'traces', 'session-delete-trace.jsonl'))).rejects.toThrow()
