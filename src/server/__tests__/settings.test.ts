...
describe('Settings API', () => {
  ...
  it('GET /api/settings/cli-launcher should expose bundled launcher status', async () => {
    ...
    expect(res.status).toBe(200)
    const body = await res.json()
    expect(body.command).toBe('loop-claw')
...
describe('Models API', () => {
  ...
  it('GET /api/models/current should prefer simple-loop-claw managed model over global user model when provider is active', async () => {
  ...
  it('PUT /api/models/current should persist to simple-loop-claw managed settings when provider is active', async () => {
    const settingsSvc = new SettingsService()
    const providerSvc = new ProviderService()
    const provider = await providerSvc.addProvider({
      presetId: 'zhipuglm',
