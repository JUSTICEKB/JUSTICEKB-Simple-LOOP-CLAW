...
describe('ConversationService', () => {
  ...
  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'simple-loop-claw-conversation-service-'))
    ...
    originalTraceApiCalls = process.env.LOOP_CLAW_TRACE_API_CALLS
    originalTraceProviderId = process.env.LOOP_CLAW_TRACE_PROVIDER_ID
    originalTraceProviderName = process.env.LOOP_CLAW_TRACE_PROVIDER_NAME
    originalTraceProviderFormat = process.env.LOOP_CLAW_TRACE_PROVIDER_FORMAT
    originalHome = process.env.HOME
    originalPath = process.env.PATH
    originalShell = process.env.SHELL
    originalZdotdir = process.env.ZDOTDIR
    originalDisableTerminalShellEnv = process.env.LOOP_CLAW_DISABLE_TERMINAL_SHELL_ENV
    ...
    delete process.env.LOOP_CLAW_TRACE_API_CALLS
    delete process.env.LOOP_CLAW_TRACE_PROVIDER_ID
    delete process.env.LOOP_CLAW_TRACE_PROVIDER_NAME
    delete process.env.LOOP_CLAW_TRACE_PROVIDER_FORMAT
    process.env.LOOP_CLAW_DISABLE_TERMINAL_SHELL_ENV = '1'
  ...
  afterEach(async () => {
    ...
    if (originalTraceApiCalls === undefined) delete process.env.LOOP_CLAW_TRACE_API_CALLS
    else process.env.LOOP_CLAW_TRACE_API_CALLS = originalTraceApiCalls

    if (originalTraceProviderId === undefined) delete process.env.LOOP_CLAW_TRACE_PROVIDER_ID
    else process.env.LOOP_CLAW_TRACE_PROVIDER_ID = originalTraceProviderId

    if (originalTraceProviderName === undefined) delete process.env.LOOP_CLAW_TRACE_PROVIDER_NAME
    else process.env.LOOP_CLAW_TRACE_PROVIDER_NAME = originalTraceProviderName

    if (originalTraceProviderFormat === undefined) delete process.env.LOOP_CLAW_TRACE_PROVIDER_FORMAT
    else process.env.LOOP_CLAW_TRACE_PROVIDER_FORMAT = originalTraceProviderFormat
    ...
    if (originalDisableTerminalShellEnv === undefined) delete process.env.LOOP_CLAW_DISABLE_TERMINAL_SHELL_ENV
    else process.env.LOOP_CLAW_DISABLE_TERMINAL_SHELL_ENV = originalDisableTerminalShellEnv
  ...
  test('keeps inherited provider env when no desktop provider config exists', async () => {
    ...
    const env = (await service.buildChildEnv('D:\\workspace\\code\\myself_code\\simple-loop-claw')) as Record<string, string>
    ...
    expect(env.CLAUDE_CODE_DIAGNOSTICS_FILE).toBe(path.join(tmpDir, 'simple-loop-claw', 'diagnostics', 'cli-diagnostics.jsonl'))
    expect(env.CLAUDE_COWORK_MEMORY_PATH_OVERRIDE).toBe(
      `${path.join(tmpDir, 'projects', 'D--workspace-code-myself-code-simple-loop-claw', 'memory')}${path.sep}`,
    )
  ...
  test('buildChildEnv inherits exported terminal shell variables for desktop CLI sessions', async () => {
    ...
    delete process.env.LOOP_CLAW_DISABLE_TERMINAL_SHELL_ENV
  ...
  test('buildChildEnv injects explicit provider runtime env for session-scoped providers', async () => {
    ...
    expect(env.CLAUDE_CODE_ATTRIBUTION_HEADER).toBe('0')
    expect(env.CLAUDE_CODE_ENTRYPOINT).toBeUndefined()
    expect(env.LOOP_CLAW_TRACE_PROVIDER_ID).toBeUndefined()
    expect(env.LOOP_CLAW_TRACE_PROVIDER_NAME).toBeUndefined()
    expect(env.LOOP_CLAW_TRACE_PROVIDER_FORMAT).toBeUndefined()
  })
  ...
  test('buildChildEnv injects trace provider metadata for desktop sdk session-scoped providers', async () => {
    ...
    expect(env.LOOP_CLAW_TRACE_PROVIDER_ID).toBe(provider.id)
    expect(env.LOOP_CLAW_TRACE_PROVIDER_NAME).toBe('Traceable Provider')
    expect(env.LOOP_CLAW_TRACE_PROVIDER_FORMAT).toBe('anthropic')
  })
  ...
  test('buildChildEnv does not inject trace env when managed trace capture is disabled', async () => {
    ...
    expect(env.LOOP_CLAW_TRACE_API_CALLS).toBeUndefined()
    expect(env.LOOP_CLAW_TRACE_PROVIDER_ID).toBeUndefined()
    expect(env.LOOP_CLAW_TRACE_PROVIDER_NAME).toBeUndefined()
    expect(env.LOOP_CLAW_TRACE_PROVIDER_FORMAT).toBeUndefined()
  })
  ...
  test('buildChildEnv injects ChatGPT Official runtime env for session-scoped provider selection', async () => {
    ...
    expect(env.LOOP_CLAW_OPENAI_OAUTH_PROVIDER).toBe('1')
    expect(env.OPENAI_CODEX_OAUTH_FILE).toBe(
      path.join(tmpDir, 'simple-loop-claw', 'openai-oauth.json'),
    )
  ...
  test('uses bun entrypoint fallback on Windows dev mode', () => {
    ...
    if (process.platform === 'win32') {
      ...
      expect(args[0]).toContain(path.join('bin', 'loop-claw'))
    }
  })
  ...
  test('buildChildEnv asks desktop SDK sessions to wait briefly for MCP tools', async () => {
    ...
    expect(env.LOOP_CLAW_DESKTOP_AWAIT_MCP).toBe('1')
    expect(env.LOOP_CLAW_DESKTOP_AWAIT_MCP_TIMEOUT_MS).toBe('5000')
  })
