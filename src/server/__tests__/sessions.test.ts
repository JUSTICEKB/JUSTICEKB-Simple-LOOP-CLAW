...
describe('SessionService', () => {
  ...
  it('should throw when workDir does not exist', async () => {
    expect(service.createSession('/tmp/definitely-missing-simple-loop-claw')).rejects.toThrow(
...
describe('Sessions API', () => {
  ...
  it('GET /api/sessions/:id/git-info should not hang when git blocks in a UTF-8 workDir', async () => {
    ...
    const { sessionId } = await sessionService.createSession(workDir)
    const oldTimeout = process.env.LOOP_CLAW_GIT_INFO_TIMEOUT_MS
    process.env.LOOP_CLAW_GIT_INFO_TIMEOUT_MS = '80'
    ...
    try {
      ...
      if (oldTimeout === undefined) {
        delete process.env.LOOP_CLAW_GIT_INFO_TIMEOUT_MS
      } else {
        process.env.LOOP_CLAW_GIT_INFO_TIMEOUT_MS = oldTimeout
      }
    }
  })
