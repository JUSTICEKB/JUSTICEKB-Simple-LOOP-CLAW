...
describe('persistent storage upgrade migrations', () => {
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'simple-loop-claw-persistence-'))
  ...
  test('imports legacy root providers config into simple-loop-claw storage without deleting the source', async () => {
    await fs.writeFile(
    ...
    expect(report.failures).toEqual([])
    expect(report.migratedEntries).toContain('providers.json -> simple-loop-claw/providers.json')
    expect(report.migratedEntries).toContain('providers.json -> simple-loop-claw/settings.json')
    expect(JSON.parse(await fs.readFile(path.join(tempDir, 'providers.json'), 'utf-8'))).toMatchObject({
      version: 1,
      activeModel: 'legacy-sonnet',
    })

    const migrated = JSON.parse(await fs.readFile(path.join(tempDir, 'simple-loop-claw', 'providers.json'), 'utf-8')) as {
      activeId?: string | null
      providerOrder?: string[]
      providers?: Array<{
        id?: string
    ...
    const managedSettings = JSON.parse(await fs.readFile(path.join(tempDir, 'simple-loop-claw', 'settings.json'), 'utf-8')) as {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        env: {
          ...
          LOOP_CLAW_SEND_DISABLED_THINKING: '1',
