...
describe('provider presets API', () => {
  ...
  test('configured presets keep current default model ids aligned with official provider docs', () => {
    ...
    expect(deepseek?.defaultEnv?.LOOP_CLAW_SEND_DISABLED_THINKING).toBeUndefined()
    ...
    expect(kimi?.defaultEnv?.LOOP_CLAW_SEND_DISABLED_THINKING).toBeUndefined()
  ...
  test('configured presets can expose optional API key and promo metadata', () => {
    ...
    expect(zhipu?.promoText).toContain('simple-loop-claw')
    expect(zhipu?.defaultEnv?.LOOP_CLAW_SEND_DISABLED_THINKING).toBeUndefined()
  ...
  test('GET and PUT /api/providers/settings read and write simple-loop-claw settings.json', async () => {
    const initial = {
      env: {
    ...
    const updatedRaw = await fs.readFile(path.join(tmpDir, 'simple-loop-claw', 'settings.json'), 'utf-8')
