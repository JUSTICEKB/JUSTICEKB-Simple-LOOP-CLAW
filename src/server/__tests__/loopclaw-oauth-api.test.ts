/**
 * Integration tests for /api/loopclaw-oauth/* endpoints.
 */
...
async function setup() {
  tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'loopclaw-oauth-api-test-'))
...
describe('POST /api/loopclaw-oauth/start', () => {
  beforeEach(setup)
  afterEach(teardown)
...
describe('GET /api/loopclaw-oauth/status', () => {
  beforeEach(setup)
  afterEach(teardown)
  ...
  test('returns loggedIn=false when stored token is expired and refresh fails', async () => {
    ...
    loopClawOAuthService.setRefreshFn(async () => {
...
describe('DELETE /api/loopclaw-oauth', () => {
  beforeEach(setup)
  afterEach(teardown)
