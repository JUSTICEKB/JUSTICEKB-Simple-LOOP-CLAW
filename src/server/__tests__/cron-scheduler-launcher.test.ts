...
const originalDisableTerminalShellEnv = process.env.LOOP_CLAW_DISABLE_TERMINAL_SHELL_ENV
const originalTaskTimeout = process.env.LOOP_CLAW_TASK_TIMEOUT_MS
...
function restoreEnv(): void {
  ...
  if (originalDisableTerminalShellEnv) {
    process.env.LOOP_CLAW_DISABLE_TERMINAL_SHELL_ENV = originalDisableTerminalShellEnv
  } else {
    delete process.env.LOOP_CLAW_DISABLE_TERMINAL_SHELL_ENV
  }
  if (originalTaskTimeout) {
    process.env.LOOP_CLAW_TASK_TIMEOUT_MS = originalTaskTimeout
  } else {
    delete process.env.LOOP_CLAW_TASK_TIMEOUT_MS
  }
...
describe('cron scheduler launcher resolution', () => {
  ...
  it('uses a configurable scheduled task timeout with the default unchanged', () => {
    ...
    expect(resolveCronTaskTimeoutMs({ LOOP_CLAW_TASK_TIMEOUT_MS: '1800000' })).toBe(1_800_000)
    expect(resolveCronTaskTimeoutMs({ LOOP_CLAW_TASK_TIMEOUT_MS: 'not-a-number' })).toBe(10 * 60 * 1000)
    expect(resolveCronTaskTimeoutMs({ LOOP_CLAW_TASK_TIMEOUT_MS: '0' })).toBe(10 * 60 * 1000)
  })

  unixOnly('executeTask arms the subprocess timeout from LOOP_CLAW_TASK_TIMEOUT_MS', async () => {
    ...
    process.env.LOOP_CLAW_TASK_TIMEOUT_MS = '12345'
  ...
  it('prefers an explicit LOOP_CLAW_ROOT when it points at a source checkout', async () => {
