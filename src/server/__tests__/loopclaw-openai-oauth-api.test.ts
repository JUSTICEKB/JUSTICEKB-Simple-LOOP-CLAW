/**
 * Integration tests for /api/loopclaw-openai-oauth/* endpoints.
 */
...
import { loopClawOpenAIOAuthService } from '../services/loopClawOpenAIOAuthService.js'
...
async function setup() {
  tmpDir = await fs.mkdtemp(
    path.join(os.tmpdir(), 'loopclaw-openai-oauth-api-test-'),
  )
...
async function teardown() {
  loopClawOpenAIOAuthService.dispose()
  loopClawOpenAIOAuthService.resetCallbackPortForTests()
...
describe('POST /api/loopclaw-openai-oauth/start', () => {
  beforeEach(setup)
  afterEach(teardown)

  test('returns authorize URL with PKCE challenge', async () => {
    const callbackPort = await getFreePort()
    loopClawOpenAIOAuthService.setCallbackPortForTests(callbackPort)
    ...
    const res = await handleLoopClawOpenAIOAuthApi(req, url, segments)
...
describe('GET /api/loopclaw-openai-oauth', () => {
  beforeEach(setup)
  afterEach(teardown)

  test('returns loggedIn=false when no token file', async () => {
    const { req, url, segments } = buildReq('GET', '/api/loopclaw-openai-oauth')
    const res = await handleLoopClawOpenAIOAuthApi(req, url, segments)
    expect(res.status).toBe(200)
  ...
  test('returns loggedIn=true + metadata when token saved', async () => {
    await loopClawOpenAIOAuthService.saveTokens({
  ...
  test('returns loggedIn=false when stored token is expired and refresh fails', async () => {
    ...
    loopClawOpenAIOAuthService.setRefreshFn(async () => {
...
describe('DELETE /api/loopclaw-openai-oauth', () => {
  beforeEach(setup)
  afterEach(teardown)
  ...
  test('clears token file', async () => {
    ...
    const { req, url, segments } = buildReq('DELETE', '/api/loopclaw-openai-oauth')
    const res = await handleLoopClawOpenAIOAuthApi(req, url, segments)
    expect(res.status).toBe(200)
    expect(await loopClawOpenAIOAuthService.loadTokens()).toBeNull()
  })
})
