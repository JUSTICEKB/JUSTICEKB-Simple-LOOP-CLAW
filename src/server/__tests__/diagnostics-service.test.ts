...
beforeEach(async () => {
  tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'simple-loop-claw-diagnostics-test-'))
...
describe('DiagnosticsService', () => {
  test('writes sanitized structured events and runtime error summaries', async () => {
    ...
    const raw = await fs.readFile(path.join(tmpDir, 'simple-loop-claw', 'diagnostics', 'diagnostics.jsonl'), 'utf-8')
    ...
    const runtime = await fs.readFile(path.join(tmpDir, 'simple-loop-claw', 'diagnostics', 'runtime-errors.log'), 'utf-8')
  ...
  test('defaults an unclassified event to info, not error', async () => {
    ...
    await expect(
      fs.readFile(path.join(tmpDir, 'simple-loop-claw', 'diagnostics', 'runtime-errors.log'), 'utf-8'),
    ).rejects.toThrow()
  })
  ...
  test('exports a single diagnostics tarball without provider secrets', async () => {
    ...
    await fs.writeFile(
      path.join(tmpDir, 'simple-loop-claw', 'diagnostics', 'cli-diagnostics.jsonl'),
...
describe('diagnostics API', () => {
  test('returns status, events, export path, and supports clearing logs', async () => {
    ...
    expect(status.logDir).toContain(path.join('simple-loop-claw', 'diagnostics'))
