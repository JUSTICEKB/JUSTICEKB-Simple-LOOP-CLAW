...
beforeAll(async () => {
  configDir = await mkdtemp(join(tmpdir(), 'simple-loop-claw-computer-use-api-'))
...
beforeEach(async () => {
  ...
  process.env.CLAUDE_CONFIG_DIR = configDir
  await rm(join(configDir, 'simple-loop-claw'), { recursive: true, force: true })
...
describe('Computer Use API authorized app config', () => {
  ...
  it('persists the Computer Use enabled flag independently', async () => {
    ...
    const raw = await readFile(
      join(configDir!, 'simple-loop-claw', 'computer-use-config.json'),
      'utf8',
    )
