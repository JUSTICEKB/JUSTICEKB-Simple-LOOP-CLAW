 ...
 * GET    /api/loopclaw-openai-oauth          — 查询当前登录状态(不回传 token 本体)
 * DELETE /api/loopclaw-openai-oauth          — 登出,删除 token 文件
 */
...
export async function handleLoopClawOpenAIOAuthApi(
  ...
  try {
    const action = segments[2] // segments: ['api', 'loopclaw-openai-oauth', <action?>]
    ...
    if (action === 'start' && req.method === 'POST') {
      ...
      const session = await loopClawOpenAIOAuthService.startSession({
    ...
    if (action === undefined && req.method === 'GET') {
      const tokens = await loopClawOpenAIOAuthService.ensureFreshTokens()
      if (!tokens) {
    ...
    if (action === undefined && req.method === 'DELETE') {
      await loopClawOpenAIOAuthService.deleteTokens()
...
export async function handleLoopClawOpenAIOAuthCallback(url: URL): Promise<Response> {
  ...
  try {
    await loopClawOpenAIOAuthService.completeSession(code, state)
...
</head><body><div class="card"><h1>✓ OpenAI Login Successful</h1><p>You can close this window and return to Simple LoopClaw.</p></div>
