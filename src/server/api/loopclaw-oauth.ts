/**
 * LoopClaw OAuth REST API
 *
 * POST   /api/loopclaw-oauth/start    — 生成 PKCE+state,返回 authorize URL
 * GET    /callback                — 用户浏览器 redirect 到此,完成 token 交换
 * GET    /api/loopclaw-oauth/callback — 兼容旧路径
 * GET    /api/loopclaw-oauth          — 查询当前登录状态(不回传 token 本体)
 * GET    /api/loopclaw-oauth/status   — 同上(legacy path)
 * DELETE /api/loopclaw-oauth          — 登出,删除 token 文件
 */
...
export async function handleLoopClawOAuthApi(
  req: Request,
  url: URL,
  segments: string[],
): Promise<Response> {
  try {
    const action = segments[2] // segments: ['api', 'loopclaw-oauth', <action?>]
    ...
    if (action === 'start' && req.method === 'POST') {
      ...
      const session = loopClawOAuthService.startSession({
    ...
    if ((action === undefined || action === 'status') && req.method === 'GET') {
      const tokens = await loopClawOAuthService.ensureFreshTokens()
      if (!tokens) {
    ...
    if (action === undefined && req.method === 'DELETE') {
      await loopClawOAuthService.deleteTokens()
...
export async function handleLoopClawOAuthCallback(url: URL): Promise<Response> {
  ...
  try {
    await loopClawOAuthService.completeSession(code, state)
...
</head><body><div class="card"><h1>✓ Login Successful</h1><p>You can close this window and return to Simple LoopClaw.</p></div>
