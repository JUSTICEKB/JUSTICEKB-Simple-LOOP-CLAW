...
describe('provider-aware thinking support', () => {
  ...
  beforeEach(() => {
    ...
    originalExplicitDisabledThinking = process.env.LOOP_CLAW_SEND_DISABLED_THINKING
  ...
  afterEach(() => {
    ...
    restoreEnv('LOOP_CLAW_SEND_DISABLED_THINKING', originalExplicitDisabledThinking)
  ...
  test('only sends explicit disabled thinking when the provider opts in', () => {
    delete process.env.LOOP_CLAW_SEND_DISABLED_THINKING
    expect(shouldSendExplicitDisabledThinking()).toBe(false)

    process.env.LOOP_CLAW_SEND_DISABLED_THINKING = '1'
