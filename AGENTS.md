...
- `bin/loop-claw` is the executable entrypoint; `bun run start` and `./bin/loop-claw` run the CLI locally.
...
- `./bin/loop-claw` or `bun run start`: run the CLI locally.
...
- Desktop Doctor and any automatic repair path must be deny-by-default. One-click repair may only mutate allowlisted, regenerable desktop UI state such as `simple-loop-claw-*` `localStorage` keys or native window state.
...
- Protected files include `~/.claude/projects/**/*.jsonl`, `~/.claude/settings.json`, project `.claude/settings.json`, `~/.claude/simple-loop-claw/providers.json`, `~/.claude/simple-loop-claw/settings.json`, `~/.claude/adapters.json`, `~/.claude/adapter-sessions.json`, `~/.claude/skills`, project `.claude/skills`, `.mcp.json`, managed MCP config, `~/.claude/plugins/**`, `~/.claude/teams/**`, and `~/.claude/simple-loop-claw/*oauth*.json`. Diagnose these paths only with redaction by default.
